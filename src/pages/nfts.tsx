...
const Nfts: NextPage = () => {
  ...
  const displayedCards = ownedNftCardProps.slice(currentPage * cardsPerPage, (currentPage + 1) * cardsPerPage);
  ...
  return (
    <div className="page" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "30px" }}>
              ...
              <TableBody>
                ...
                <TableRow>
                  ...
                  <TableCell className={styles.table}>
                    {Math.floor(
                      (((1 / Number(totalStakedState)) * 7500000 * 365 * ((reservesEth / reservesProphet) * ethInUSD)) /
                        25) *
                        100
                    )}
                    %
      ...
      <NftApproveAndActionCard
        mounted={mounted}
        isConnected={isConnected}
        cardTitle={"Approve burning $PROPHET"}
        amountToApprove={Number(toWei(mintCount, "ether")) * 400000.01}
        allowanceAmount={Number(currentAllowanceState)}
        mintCount={mintCount}
        totalMinted={totalMinted}
        setTotalMinted={setTotalMinted}
        setOwnedNftCardProps={setOwnedNftCardProps}
        setStateAllowanceAmount={setStateAllowanceAmount}
      ></NftApproveAndActionCard>
      <Typography className="container" variant="h3">
        NFTs at 0x{String(address).slice(-4)}
      </Typography>
      <div>
        <Button
          color="secondary"
